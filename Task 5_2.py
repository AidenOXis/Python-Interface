...
class IsulinometroApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.stored_values = []  # Dati campionati da plottare
    ...
    def validate_values(self, values, min_val, max_val):
        try:
            values = [float(val) for val in values]  # Convert all inputs to floats
            if all(min_val <= val <= max_val for val in values):
                return values
            else:
                raise ValueError(f"Values should be between {min_val} and {max_val}")
        except ValueError as e:
            messagebox.showerror("Invalid input", str(e))
            return None

    def input_values_popup(self):
        # Popup asking for a list of values
        input_values = simpledialog.askstring("Input", "Inserire una lista di valori campionati (separati da spazi).\nI valori devono rientrare nel range di 1-500 Ohm, saranno disposti con un passo di campionamento simulato pari a 1 secondo.")

        if input_values:  # If input is not empty
            values_list = input_values.split()  # Split the input by spaces

            # Define acceptable range for values
            min_val, max_val = 1, 500  
            valid_values = app.validate_values(values_list, min_val, max_val)

            if valid_values:
                self.stored_values = valid_values  # Now stored_values contains the valid values
                print(f"Stored values: {self.stored_values}")  # For debugging purposes
    
    def plot_graph(self):
        if self.stored_values:
            plt.plot(self.stored_values)  # Simple line plot
            plt.title("Plot of Stored Values")
            plt.xlabel("Index")
            plt.ylabel("Values")
            plt.show()
        else:
            messagebox.showwarning("No Data", "No valid values to plot!")
